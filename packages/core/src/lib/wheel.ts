...
import { removeEnter, hasBackground, computeRange, splitText, has } from '../utils/index'
...
export default class LuckyWheel extends Lucky {
    ...
    this.prizes.forEach((prize, prizeIndex) => {
      ...
      prize.imgs &&
        prize.imgs.forEach((imgInfo, imgIndex) => {
          ...
          const isShowBorder = imgInfo.isShowBorder || false
          this.drawRoundAvatar(
            ctx,
            prizeImg,
            trueWidth / 2,
            isShowBorder,
  ...
  private carveOnGunwaleOfAMovingBoat(): void {
    ...
    const stopRange =
      (Math.random() * prizeDeg - prizeDeg / 2) * this.getLength(_defaultConfig.stopRange)
    let i = 0,
      prevSpeed = 0,
      prevDeg = 0
    while (++i) {
      const endDeg =
        ...
        prizeDeg / 2
      let currSpeed =
        quad.easeOut(this.FPS, stopDeg, endDeg, _defaultConfig.decelerationTime) - stopDeg
      if (currSpeed > speed) {
